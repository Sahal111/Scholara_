      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#bfc9c4]/20 bg-[#f2f4f3]">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center border ${ctx.bg} ${ctx.border}`}
            >
              <span
                className={`material-symbols-outlined text-[22px] ${ctx.icon_c}`}
            ...
            <div>
              ...
              <p className="text-xs text-[#707975] max-w-xs">
                {isEdit
        ...
        {!isEdit && isParentLocked && parentJenis && (
          <div className="px-6 pt-4">
            <div
              className={`flex items-center gap-2 px-4 py-3 rounded-xl border ${ctx.bg} ${ctx.border}`}
            >
              <span
                className={`material-symbols-outlined text-[16px] ${ctx.icon_c}`}
              >
                account_tree
              </span>
              <div className="flex items-center gap-1.5 flex-1 min-w-0">
                ...
                <span className={`text-sm font-bold truncate ${ctx.text}`}>
                  {defaultParentLabel ?? "Program terpilih"}
                </span>
              </div>
              <span
                className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full whitespace-nowrap ${ctx.badge} ${ctx.text}`}
              >
                Induk
              </span>
            </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
          ...
          <div>
            ...
            {isJenisLocked ? (
              <div
                className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl border ${ctx.bg} ${ctx.border}`}
              >
                <span
                  className={`material-symbols-outlined text-[18px] ${ctx.icon_c}`}
                  style={{ fontVariationSettings: "'FILL' 1" }}
                >
                  {ctx.icon}
                </span>
                <span className={`text-sm font-bold ${ctx.text}`}>
                  {jenisLabel}
                </span>
              </div>
            ) : (
          ...
          <div>
            ...
            <input
              ...
              maxLength={10}
            />
            <p className="mt-1 text-[11px] text-[#707975]">{ctx.kodeHint}</p>
          </div>
          {/* Jenjang */}
          <div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[#bfc9c4]/20 bg-[#f2f4f3]">
          ...
          <button
            ...
            disabled={isPending}
            className="px-6 py-2.5 rounded-xl text-white text-sm font-bold disabled:opacity-60 flex items-center gap-2 bg-[#006e2a] hover:bg-[#00531e] transition-all shadow-sm"
          >
            {isPending ? (
              <>
                <svg
