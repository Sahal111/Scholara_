...
const ALERTS = [
  ...
  },
  {
    ...
    btnCls:
      "border-amber-200 text-amber-800 hover:bg-amber-500 hover:text-white",
    icon: "description",
  ...
  },
  {
    ...
    btnCls:
      "border-purple-200 text-purple-700 hover:bg-purple-500 hover:text-white",
    icon: "schedule",
...
const QUICK_ACTIONS = [
  ...
  { icon: "balance", label: "Review Beban Mengajar", to: "/wakasek/guru" },
  {
    ...
    label: "Validasi Jadwal & Ruang",
    to: "/wakasek/kelas",
  },
  {
    icon: "task",
    label: "Monitor Penilaian e-Rapor",
    to: "/wakasek/nilai",
    disabled: true,
  },
  {
    icon: "assessment",
    label: "Lihat Laporan Akademik",
    to: "/wakasek/laporan",
  },
];
...
function PillarCard({ d }) {
  return (
    <div
      className={`p-3.5 rounded-xl bg-slate-50/80 border border-slate-200/70 ${d.hoverBg} flex flex-col justify-between transition-all duration-200 group shadow-sm hover:shadow`}
    >
      <div>
        <div className="flex items-center justify-between">
          <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
            <span className={`w-1.5 h-1.5 rounded-full ${d.dotColor}`} />{" "}
            {d.label}
          </span>
          <div
            className={`w-7 h-7 rounded-lg ${d.iconBg} flex items-center justify-center border transition-transform group-hover:scale-110`}
          >
        ...
        <div className="mt-2.5 flex items-baseline justify-between gap-1">
          ...
          <span
            className={`text-[10px] font-bold ${d.badgeColor} px-2 py-0.5 rounded-md border`}
          >
            {d.badge}
          </span>
        </div>
        <div className="mt-2.5 w-full bg-slate-100 h-1.5 rounded-full overflow-hidden border border-slate-200/40">
          <div
            className={`${d.barColor} h-full rounded-full transition-all duration-500`}
            style={{ width: d.width }}
          />
        </div>
      </div>
      <span
        className={`text-[10px] ${d.subColor || "text-slate-400"} mt-2 font-medium`}
      >
        {d.sub}
      </span>
    </div>
...
function AlertCard({ a }) {
  return (
    <div
      className={`p-3.5 rounded-xl border ${a.colors} bg-gradient-to-r to-white hover:shadow-sm transition-all flex items-start gap-3 group`}
    >
      <div
        className={`w-8 h-8 rounded-xl ${a.countBg} font-extrabold text-xs flex items-center justify-center shrink-0 border shadow-sm`}
      >
        {a.count}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-1.5">
          ...
          <span
            className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${a.badgeCls} uppercase tracking-wide shrink-0`}
          >
            {a.badge}
          </span>
        </div>
        ...
        <div className="mt-2">
          <button
            className={`px-2.5 py-1 rounded-lg bg-white border ${a.btnCls} text-[11px] font-semibold transition-all shadow-sm inline-flex items-center gap-1.5`}
          >
...
function AgendaCard({ a }) {
  return (
    <div
      className={`p-3.5 rounded-xl border ${a.borderColor} ${a.bg} flex flex-col justify-between`}
    >
      <div>
        <div className="flex items-center justify-between mb-2">
          <span
            className={`px-2 py-0.5 rounded-md ${a.dateBg} text-[11px] font-bold`}
          >
            {a.date}
          </span>
          <span
            className={`text-[10px] font-bold ${a.tagCls} px-1.5 py-0.5 rounded`}
          >
            {a.tag}
          </span>
        </div>
      ...
      <div className="mt-3 pt-2 border-t border-slate-200/50 flex items-center justify-between text-[10px] text-slate-400">
        ...
        <span className={`font-semibold ${a.metaColor || "text-slate-500"}`}>
          {a.meta}
        </span>
      </div>
    </div>
...
export default function DashboardWakasek() {
  ...
  const labelSemester =
    semesterAktif && taAktif
      ? `Semester ${semesterAktif.nama} ${taAktif.tahun}`
      : "Semester Aktif";
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-600 border border-blue-200 shadow-sm">
                ...
                <Icon name="terminal" className="text-[14px]" /> Academic
                Command Center
              </span>
            </div>
            <p className="text-xs text-slate-500 max-w-2xl leading-relaxed">
              Pantau kesehatan akademik, pelaksanaan pembelajaran harian, dan
              capaian target kurikulum sekolah secara terpusat dan real-time.
            </p>
          ...
          <div className="flex items-center gap-5 xl:pr-8 xl:border-r border-slate-200/80 flex-shrink-0">
            <div className="relative flex items-center justify-center shrink-0 p-2 rounded-2xl bg-gradient-to-br from-blue-50/70 to-slate-100 border border-blue-100 shadow-sm">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center leading-none">
                ...
                <span className="text-[8px] font-bold text-blue-600 tracking-wider uppercase px-2 py-0.5 rounded-full bg-blue-100/80 mt-1 border border-blue-200/60">
                  Indeks
                </span>
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-slate-500 leading-relaxed max-w-sm">
                Konsolidasi 4 pilar mutu KBM harian & kepatuhan regulasi
                kalender akademik.
              </p>
              <div className="flex items-center gap-2 pt-0.5 text-xs flex-wrap">
                <span className="inline-flex items-center gap-1 text-[11px] font-medium text-slate-400 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-200/70">
                  Target Rapor Pendidikan:{" "}
                  <strong className="font-bold text-slate-700 ml-1">
                    ≥ 90.0
                  </strong>
                </span>
          ...
          <div className="relative">
              ...
              <div>
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-[11px] font-semibold bg-blue-50 text-blue-600 border border-blue-100 px-2.5 py-0.5 rounded-full inline-flex items-center gap-1.5 shadow-sm">
                    <Icon name="monitor_heart" className="text-[13px]" /> Live
                    Sensor
                  </span>
                </div>
                <p className="text-xs text-slate-500 mt-1">
                  Performa dan dinamika fluktuasi operasional KBM dalam 7 hari
                  terakhir.
                </p>
              </div>
              <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-xl border border-slate-200/70 self-start sm:self-auto">
                <button className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-white text-blue-600 shadow-sm">
                  7 Hari
                </button>
                <button className="px-2.5 py-1 rounded-lg text-xs font-medium text-slate-500 hover:text-slate-800 transition-colors">
                  Pekan Ini
                </button>
                <button className="px-2.5 py-1 rounded-lg text-xs font-medium text-slate-500 hover:text-slate-800 transition-colors">
                  Bulan Ini
            ...
            <div className="relative w-full h-48 sm:h-56 mt-2">
              ...
              >
                ...
                <path
                  d="M 65,96 C 145,86 185,68 265,58 C 345,50 425,46 505,48 C 585,50 635,62 665,65 L 665,185 L 65,185 Z"
                  fill="url(#pulsePrimaryGrad)"
                />
                <path
                  d="M 65,48 C 145,40 185,34 265,30 C 345,34 425,36 505,37 C 585,40 635,44 665,46 L 665,185 L 65,185 Z"
                  fill="url(#pulseSuccessGrad)"
                />
                <line
                  x1="265"
                  y1="15"
                ...
                />
                <path
                  d="M 65,152 C 145,142 185,134 265,128 C 345,118 425,110 505,106 C 585,104 635,102 665,100"
                  fill="none"
                  stroke="#8B5CF6"
                  strokeWidth="2"
                  strokeDasharray="5 4"
                ...
                <path
                  d="M 65,48 C 145,40 185,34 265,30 C 345,34 425,36 505,37 C 585,40 635,44 665,46"
                  fill="none"
                  stroke="#10B981"
                  strokeWidth="2.5"
                  strokeLinecap="round"
                ...
                />
                <path
                  d="M 65,96 C 145,86 185,68 265,58 C 345,50 425,46 505,48 C 585,50 635,62 665,65"
                  fill="none"
                  stroke="#2563EB"
                  strokeWidth="3"
                  strokeLinecap="round"
              ...
              <div className="absolute left-[37.8%] top-1 -translate-x-1/2 bg-slate-900/95 backdrop-blur-md text-white px-3.5 py-2 rounded-xl shadow-xl border border-slate-700/60 text-xs pointer-events-none z-10 hidden sm:block">
                ...
                <div className="flex items-center gap-2.5 text-[11px]">
                  <span className="text-blue-400 font-semibold">
                    KBM: <strong className="text-white font-bold">98.4%</strong>
                  </span>
                  <span className="text-slate-600">·</span>
                  <span className="text-emerald-400 font-semibold">
                    Hadir:{" "}
                    <strong className="text-white font-bold">99.1%</strong>
                  </span>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-7 text-center text-xs text-slate-400 mt-2 px-2 sm:px-8 font-medium">
              {[
                ...
                <span
                  ...
                  className={
                    d === "Rabu"
                      ? "font-bold text-blue-600 bg-blue-50/70 py-0.5 rounded-md"
                      : ""
                  }
                >
                  {d}
            ...
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-200/60">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Hal yang membutuhkan keputusan & disposisi hari ini.
                </p>
              </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 lg:p-6 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all duration-300 flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="flex items-start justify-between pb-3.5 border-b border-slate-200/60">
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-[11px] font-medium text-slate-400 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-200/40">
                    Hari Ini
                  </span>
                </div>
                ...
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Monitoring kehadiran harian seluruh rombel
                </p>
              </div>
              ...
              {[
                {
                  ...
                  pct: "98.2%",
                  dot: "bg-emerald-500",
                  pctCls: "text-emerald-700 bg-emerald-100/70",
                },
                {
                  label: "Izin/Sakit",
                  value: "14",
                  pct: "1.1%",
                  dot: "bg-amber-400",
                  pctCls: "text-amber-800 bg-amber-100/70",
                ...
                },
                {
                  ...
                  pct: "0.7%",
                  dot: "bg-rose-500",
                  pctCls: "text-rose-700 bg-rose-100/70",
                  valueColor: "text-rose-500",
                },
              ].map((s) => (
                <div
                  key={s.label}
                  className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/60 text-center shadow-sm"
                >
                  ...
                  <span
                    className={`text-sm font-extrabold ${s.valueColor || "text-slate-800"} block leading-tight`}
                  >
                    {s.value}
                  </span>
                  <span
                    className={`text-[9px] font-semibold ${s.pctCls} px-1.5 py-0.5 rounded-full inline-block mt-1`}
                  >
                    {s.pct}
                  </span>
                </div>
          ...
          <div className="mt-4 pt-3.5 border-t border-slate-200/60 flex items-center justify-between text-[11px]">
            <span className="inline-flex items-center gap-1.5 text-emerald-700 font-medium bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100/70 shadow-sm">
              <Icon name="check_circle" className="text-[13px]" /> 98% Kelas
              Tervalidasi
            </span>
            <Link
              to="/wakasek/absensi"
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 lg:p-6 shadow-sm hover:shadow-md hover:border-blue-200 transition-all duration-300 flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="space-y-2 mt-4">
              ...
              <div className="flex items-center justify-between p-2.5 rounded-xl bg-blue-50/40 border border-blue-100/70 text-xs shadow-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-blue-700 bg-blue-100 px-2 py-0.5 rounded-md font-semibold border border-blue-200/70">
                    9.5%
                  </span>
                </div>
              </div>
              <div className="flex items-center justify-between p-2.5 rounded-xl bg-amber-50/40 border border-amber-100/70 text-xs shadow-sm">
                <span className="flex items-center gap-2 text-amber-800 font-medium">
                  <span className="w-2 h-2 rounded-full bg-amber-400" /> Perlu
                  Konfirmasi
                </span>
                <span className="text-[11px] font-bold text-amber-700 bg-amber-100/70 px-2.5 py-0.5 rounded-full border border-amber-200/60">
                  2 sesi
                </span>
              </div>
            </div>
          ...
          <div className="mt-4 pt-3.5 border-t border-slate-200/60 flex items-center justify-between text-[11px]">
            <span className="inline-flex items-center gap-1.5 text-emerald-700 font-medium bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100/70">
              <Icon name="verified_user" className="text-[13px]" /> 0 Jam Kosong
              Terlantar
            </span>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 lg:p-6 shadow-sm hover:shadow-md hover:border-purple-200 transition-all duration-300 flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="mt-4 p-3.5 rounded-xl bg-slate-50 border border-slate-200/60 flex items-center gap-4 shadow-sm">
              <div className="relative flex items-center justify-center shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center leading-none">
                  ...
                  <span className="text-[8px] font-semibold text-purple-700 uppercase tracking-wider mt-0.5">
                    Input
                  </span>
                </div>
              </div>
                ...
                {[
                  {
                    ...
                    dot: "bg-purple-500",
                    pct: "78%",
                    pctCls: "text-purple-700 bg-purple-50 border-purple-100",
                  },
                  {
                    label: "Dalam Proses",
                    value: "20 Kelas",
                    dot: "bg-blue-500",
                    pct: "14%",
                    pctCls: "text-blue-700 bg-blue-50 border-blue-100",
                  },
                  {
                    label: "Perlu Input",
                    value: "12 Kelas",
                    ...
                    pct: "8%",
                    pctCls: "text-amber-800 bg-amber-100 border-amber-200",
                  ...
                  >
                    <span
                      className={`${m.labelColor || "text-slate-500"} flex items-center gap-1.5 font-medium`}
                    >
                    ...
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`font-bold ${m.valueColor || "text-slate-700"}`}
                      >
                        {m.value}
                      </span>
                      <span
                        className={`text-[10px] font-semibold ${m.pctCls} px-1.5 py-0.5 rounded border`}
                      >
                        {m.pct}
                      </span>
                    </div>
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="truncate">
                  <p className="text-[11px] font-semibold text-amber-800 leading-tight">
                    Batas Cut-Off:{" "}
                    <span className="font-extrabold">H-3 (11 Okt)</span>
                  </p>
                  <p className="text-[10px] text-slate-400 leading-tight">
                    Sisa 3 hari kerja sebelum sinkronisasi
                  </p>
                </div>
              </div>
          ...
          <div className="mt-4 pt-3.5 border-t border-slate-200/60 flex items-center justify-between text-[11px]">
            <span className="text-amber-700 font-medium inline-flex items-center gap-1.5 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-100/70">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-400" /> 12
              kelas butuh verifikasi
            </span>
      ...
      <section className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200/50">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Linimasa jadwal asesmen, koordinasi supervisi klinis, dan batas
              cut-off kurikulum.
            </p>
          </div>
          <Link
            to="/wakasek/kalender"
            className="text-xs font-semibold text-blue-600 hover:underline flex items-center gap-1 self-start sm:self-auto"
          >
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-4 bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Pintasan tugas pimpinan kurikulum dan tata kelola pembelajaran.
            </p>
            <div className="space-y-2 mt-4">
              {QUICK_ACTIONS.map((q) =>
                q.primary ? (
                  <Link
                    key={q.label}
                    to={q.to}
                    className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl bg-blue-600 text-white text-xs font-semibold hover:bg-blue-700 transition-colors shadow-sm"
                  >
                    <span className="flex items-center gap-2">
                  ...
                  <div
                    key={q.label}
                    className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-medium text-slate-300 cursor-not-allowed"
                  >
                    <span className="flex items-center gap-2">
                      <Icon name={q.icon} className="text-[16px]" /> {q.label}
                    </span>
                  ...
                  <Link
                    key={q.label}
                    to={q.to}
                    className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-xs font-medium text-slate-600 transition-colors"
                  >
        ...
        <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between pb-3 border-b border-slate-200/50">
              ...
              <span className="text-[11px] font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">
                Kompilasi Scholara AI
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-1">
              Ringkasan intelijen operasional untuk dasar pengambilan keputusan
              pimpinan kurikulum.
            </p>
            <div className="space-y-3 mt-4">
              {[
                  ...
                  text: (
                    <>
                      ...
                      <strong className="text-emerald-600 font-semibold">
                        1.8% dibanding minggu lalu
                      </strong>{" "}
                      (kini mencapai 98.2%). Didorong oleh efektivitas program
                      bimbingan wali kelas dan presensi digital real-time.
                    </>
                  ),
                },
                {
                  ...
                  text: (
                    <>
                      <strong className="text-blue-600 font-semibold">
                        92% guru telah mengisi jurnal pembelajaran
                      </strong>{" "}
                      tepat waktu pada hari pelaksanaan. Ketercapaian Alur
                      Tujuan Pembelajaran (ATP) terjaga pada standar 94.8%.
                    </>
                  ),
                },
                {
                  ...
                  text: (
                    <>
                      <strong className="text-amber-600 font-semibold">
                        12 kelas masih membutuhkan tindak lanjut penilaian
                      </strong>{" "}
                      sumatif tengah semester. Batas akhir cut-off e-Rapor
                      tersisa 3 hari kerja sebelum sinkronisasi.
                    </>
                  ),
                },
              ].map((insight) => (
                <div
                  key={insight.title}
                  className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/60 flex items-start gap-3.5"
                >
                  <div
                    className={`w-8 h-8 rounded-lg ${insight.iconBg} flex items-center justify-center shrink-0`}
                  >
          ...
          <div className="mt-4 pt-3 border-t border-slate-200/50 flex items-center justify-between text-[11px] text-slate-500">
            <span>
              Saran Sistem: Kirim rekapitulasi evaluasi otomatis ke forum MGMP
              sekolah.
            </span>
            <button className="text-blue-600 font-semibold hover:underline">
              Ekspor Ringkasan Eksekutif →
            </button>
          </div>
        </div>
      </section>
