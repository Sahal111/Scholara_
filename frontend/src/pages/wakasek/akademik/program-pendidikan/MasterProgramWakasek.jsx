...
function StatCard({ icon, label, value, unit }) {
  return (
    <div className="bg-white rounded-[2rem] p-8 border border-[#c4b5fd]/20 shadow-sm group relative overflow-hidden transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_10px_25px_-5px_rgba(109,40,217,0.1)]">
      <div className="absolute top-0 right-0 w-32 h-32 bg-[#7c3aed]/5 rounded-full -mr-16 -mt-16 transition-transform duration-500 group-hover:scale-150 pointer-events-none" />
      <div className="relative z-10 flex items-center gap-6">
        <div className="w-16 h-16 rounded-2xl bg-[#7c3aed]/10 flex items-center justify-center text-[#7c3aed] group-hover:bg-[#7c3aed] group-hover:text-white transition-all duration-300 flex-shrink-0">
          <span className="material-symbols-outlined text-3xl">{icon}</span>
        </div>
        <div>
          <p className="text-xs font-bold text-[#7c3aed] uppercase tracking-[0.2em] mb-1">
            {label}
          </p>
          <div className="flex items-baseline gap-2">
            <h3
              className="text-4xl font-extrabold text-[#1e1b4b] tracking-tighter"
              style={{ fontFamily: "'Plus Jakarta Sans',sans-serif" }}
            >
              {value}
            </h3>
            <span className="text-sm font-medium text-[#4c1d95]/60">
              {unit}
            </span>
          </div>
        </div>
...
function JenjangEmptyState({ school }) {
  return (
    <div className="bg-white rounded-[2rem] shadow-sm border border-[#c4b5fd]/20 overflow-hidden">
      <div className="flex flex-col items-center justify-center text-center gap-5 py-24 px-8">
        <div className="w-20 h-20 rounded-full bg-[#f5f3ff] border border-[#c4b5fd]/30 flex items-center justify-center">
          <span className="material-symbols-outlined text-5xl text-[#c4b5fd]">
            school
          </span>
        </div>
        <div>
          ...
          <p className="text-sm text-[#6d28d9]/60 max-w-sm leading-relaxed">
            Jenjang{" "}
...
/* ─── MasterProgramWakasek ────────────────────────────────────────────────── */
export default function MasterProgramWakasek() {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12 relative">
        <div className="relative flex-1">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="px-4 py-1.5 rounded-full bg-[#7c3aed]/10 border border-[#7c3aed]/20 flex items-center gap-2 shadow-sm">
              <span className="w-2 h-2 rounded-full bg-[#7c3aed] animate-pulse" />
              <span
                className="text-[10px] text-[#7c3aed] tracking-[0.2em] uppercase font-black"
                style={{ fontFamily: "'Inter',sans-serif" }}
              >
                KEBIJAKAN AKADEMIK
              </span>
            </div>
            <div className="h-px w-32 bg-gradient-to-r from-[#7c3aed]/20 to-transparent" />
          </div>

          {/* Title */}
          <h1
            className="text-[48px] font-extrabold text-[#1e1b4b] leading-tight tracking-tighter mb-3"
            style={{ fontFamily: "'Plus Jakarta Sans',sans-serif" }}
          >
            Program{" "}
            <span
              className="italic font-normal text-[#7c3aed]"
          ...
          <p
            className="text-lg text-[#4c1d95]/70 max-w-2xl leading-relaxed"
            style={{ fontFamily: "'Inter',sans-serif" }}
          >
        ...
        {programConfig.hasTabs && (
          <div className="shrink-0 flex items-center gap-3 w-full md:w-auto">
            {/* Recycle Bin — path ke wakasek */}
            <Link
              to="/wakasek/program-pendidikan/recycle-bin"
              className="flex items-center gap-2 px-5 py-3.5 rounded-full bg-white/80 border border-[#c4b5fd]/40 text-[#4c1d95] hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-all shadow-sm group"
              title="Recycle Bin"
            >
            ...
            {canManage && (
              <button
                ...
                className="flex-1 md:flex-none bg-[#7c3aed] text-white px-8 py-4 rounded-full flex items-center justify-center gap-3
                  shadow-[0_8px_16px_rgba(124,58,237,0.20)] hover:shadow-[0_15px_40px_rgba(124,58,237,0.45)]
